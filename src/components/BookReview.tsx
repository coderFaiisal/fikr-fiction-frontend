      ...
      <form className="flex gap-5 items-center" onSubmit={handleSubmit}>
        ...
        {isLoading ? (
          ...
          <button type="submit" className="rounded-full text-[25px]">
            <PaperAirplaneIcon className=" -rotate-45 bg-purple-500 text-white w-8 lg:w-12 rounded-full p-2" />
          </button>
        )}
      </form>
      <div className="mt-10">
        {reviews?.map((review) => (
          <div key={review?._id} className="flex gap-3 items-center mb-5">
            ...
            <div>
              <p className="font-semibold text-xs md:text-lg">{review?.userName}</p>
